...
`r as.numeric(num[2,2])` subjects for L1 and `r as.numeric(num[1,2])`
subjects for L2 for a total of `r dim(DLM)[1]` observations left. The
...
lmm.2 <- update(lmm.0, .~. + LangLev)
anova(lmm.0,lmm.2)

lmm.3 <- update(lmm.0, .~. + LangLev:Type)
anova(lmm.0,lmm.3)

lmm.4 <- update(lmm.0, .~. + List)
anova(lmm.0,lmm.4)
```
