...
options(formatR.arrow=TRUE, width=80, digits=5,show.signif.stars = TRUE)
library(ggplot2)
library(dplyr)
library(lme4)
#library(lmerTest)
...
print(xtable(summary(lmm,transform="log")),type="html")
```
...
Note that due to the removal of entries with missing values, our experimental set up is not balanced. Hence, we do not report p-values for F or t statistics as there are no analytical results for null distributions of parameter estimates in complex situations (e.g., unbalanced or partially crossed designs) [Bates 2015].
